...
function CredentialForm({
  ...
  // window.location is only available client-side; read it once after mount
  // so the correct deployed origin shows up instead of a server placeholder.
  const [origin, setOrigin] = useState("");
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- one-time sync from a browser-only API, not derivable during SSR.
    setOrigin(window.location.origin);
  }, []);
  const webhookUrl = `${origin}/api/webhook/${agentId}`;
